...
function getStyle(obj,attr)
{
  ...
  return arr;
    ...
    {   var last=getChild(obj,type).length-1;
        return getChild(obj,type)[last];
    }
     // 获得元素任何一个子元素
   ...
   {   type=type==undefined?true:type
    ...
    function addEvent(obj,even,fun)
    {
      if(obj.addEventListener){
           obj.addEventListener(even,fun,false);
            }
      else{
           obj.attachEvent("on"+even,fun);
            }   
    }
    ...
    function delEvent(obj,even,fun){
     if(obj.removeEventListener){
          obj.removeEventListener(even,fun,false);
              }
        else{
         obj.detachEvent("on"+even,fun); 
             }
     }
     // 添加滚轮事件
